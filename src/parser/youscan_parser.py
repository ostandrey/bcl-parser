    ...
    async def _parse_user_description_async(self, entry_elem, name_elem) -> str:
        ...
        try:
            ...
            if clicked:
                ...
                # Look for modal/popup — try both role="dialog" and role="presentation"
                # (Instagram popups use role="presentation", Facebook uses role="dialog")
                # The description is in a div with class like "jrnY5tmnFg128QMOKSyq"
                modal = None
                modal_selector = '[role="dialog"], [role="presentation"][class*="awnQnu0ms"]'
                try:
                    ...
                    # Fallback: Instagram uses role="presentation" for its popup
                    try:
                    ...
                    if not modal:
                        try:
                            modal = await self.page.query_selector('[role="presentation"][class*="awnQnu0ms"]')
                            if modal:
                                logger.debug("Found Instagram-style popup (role=presentation)")
                                print("[DESC] Found Instagram-style popup (role=presentation)")
                        except:
                            pass
                ...
                if modal:
                    ...
                    description_selectors = [
                        'div[class*="jrnY5tmnFg128QMOKSyq"]',  # Main selector (Facebook & Instagram)
                    ...
                    if not desc_text:
                        logger.debug("[DESC] No description found in popup (element empty or absent)")
                        print("[DESC] No description found — leaving empty")
